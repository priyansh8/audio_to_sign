...
#         VBZ Present 3rd person singular (takes)
		...
		interroWords = ["where","who","how", "what", "whome", "which"]
		for w in range(len(words)):
			if(words[w] in interroWords):
				temp = words[w]
				del words[w]
				words.append(temp)

		'''OutputText = " ".join(words)
		
		
		
		file = pathlib.Path("dataset.pkl")
		if file.exists():
			dbfile = open('dataset.pkl', 'ab')
			db = pickle.load(dbfile)    
			db[inputText] = OutputText
			pickle.dump(db, dbfile)                      
    		dbfile.close()
			
		else:
			dbfile = open('dataset.pkl', 'wb')
			db = {}
			db[inputText] = OutputText
			pickle.dump(db, dbfile)                      
    		dbfile.close()'''
